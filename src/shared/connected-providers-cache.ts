...
function getCacheFilePath(filename: string): string {
	return join(dataPath.getOcoOpenCodeCacheDir(), filename)
}
...
function ensureCacheDir(): void {
	const cacheDir = dataPath.getOcoOpenCodeCacheDir()
...
export function hasConnectedProvidersCache(): boolean {
	return getReadableCacheFilePaths(CONNECTED_PROVIDERS_CACHE_FILE).some(existsSync)
}
...
export function hasProviderModelsCache(): boolean {
	return getReadableCacheFilePaths(PROVIDER_MODELS_CACHE_FILE).some(existsSync)
}
