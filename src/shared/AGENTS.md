...
Cross-cutting utilities used throughout the plugin. Barrel-exported from `index.ts`. Logger writes to `/tmp/opencode-codex-orch.log`.
...
- `agent-names.ts`: Remaining legacy aliases → canonical names
...
|---------|-------------|---------|
| `logger.ts` | 62 | `/tmp/opencode-codex-orch.log` |
