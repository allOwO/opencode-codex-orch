/**
 * Detects external plugins that may conflict with opencode-codex-orch features.
...
/**
 * Known notification plugins that conflict with opencode-codex-orch's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[opencode-codex-orch] External notification plugin detected: ${pluginName}

Both opencode-codex-orch and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   opencode-codex-orch's session-notification has been auto-disabled.

   To use opencode-codex-orch's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in opencode-codex-orch.json`
}
