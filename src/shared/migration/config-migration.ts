...
function migrateRemovedCategories(record: Record<string, unknown>): {
  ...
  for (const [key, value] of Object.entries(record)) {
    const nextKey = REMOVED_CATEGORY_MAP[key]
    if (!nextKey) continue

    const nextValue = migrated[nextKey]
    const valueRecord = value && typeof value === "object" ? value as Record<string, unknown> : undefined
    const nextRecord = nextValue && typeof nextValue === "object" ? nextValue as Record<string, unknown> : undefined
    migrated[nextKey] = nextRecord ? { ...valueRecord, ...nextRecord } : value
    delete migrated[key]
    changed = true
  }
...
function migrateAgentCategoryReferences(record: Record<string, unknown>): boolean {
  let changed = false

  for (const value of Object.values(record)) {
    if (!value || typeof value !== "object") continue
    const config = value as Record<string, unknown>
    if (typeof config.category !== "string") continue

    const nextCategory = REMOVED_CATEGORY_MAP[config.category]
...
export function migrateConfigFile(
  ...
  if (copy.agents && typeof copy.agents === "object") {
    ...
    for (const key of Object.keys(copy.agents as Record<string, unknown>)) {
      if (REMOVED_AGENT_NAMES.has(key.toLowerCase())) {
        delete (copy.agents as Record<string, unknown>)[key]
        needsWrite = true
      }
    }

    if (migrateAgentCategoryReferences(copy.agents as Record<string, unknown>)) {
      needsWrite = true
    }
  }
  ...
  if (copy.categories && typeof copy.categories === "object") {
    const { migrated: migratedCategories, changed: categoryNamesChanged } = migrateRemovedCategories(
      copy.categories as Record<string, unknown>
    )
    if (categoryNamesChanged) {
      copy.categories = migratedCategories
      needsWrite = true
    }
  ...
  if (copy.disabled_agents && Array.isArray(copy.disabled_agents)) {
    ...
    for (const agent of copy.disabled_agents as string[]) {
      ...
      if (REMOVED_AGENT_NAMES.has(newAgent.toLowerCase())) {
        changed = true
        continue
      }
      if (newAgent !== agent) {
