...
describe("external-plugin-detector", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oco-test-"))
  })
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only opencode-codex-orch is configured", () => {
      // given - opencode.json with only opencode-codex-orch
      const opencodeDir = path.join(tempDir, ".opencode")
      fs.mkdirSync(opencodeDir, { recursive: true })
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["opencode-codex-orch"] })
      )
      ...
      expect(result.detected).toBe(false)
      expect(result.pluginName).toBeNull()
      expect(result.allPlugins).toContain("opencode-codex-orch")
    })
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["opencode-codex-orch", "opencode-notifier"] })
      )
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["opencode-codex-orch", "opencode-notifier@1.2.3"] })
      )
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["opencode-codex-orch", "@mohak34/opencode-notifier"] })
      )
