...
describe("session-recovery storage SDK readers", () => {
  ...
  it("readMessagesFromSDK reads nested info metadata", async () => {
    //#given a client that returns SDK messages with nested info metadata
    const sessionID = "ses_test"
    const client = createMockClient({
      messages: () => [
        { info: { id: "msg_b", role: "assistant", time: { created: 2 } } },
    ...
    //#then it normalizes nested metadata the same way
    expect(result).toEqual([
      { id: "msg_a", sessionID, role: "user", time: { created: 1 } },
      { id: "msg_b", sessionID, role: "assistant", time: { created: 2 } },
    ])
