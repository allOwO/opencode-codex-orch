...
20 files. Stateless utility functions for the `install` command. Handles OpenCode config manipulation, provider configuration, config operations, binary detection, and npm registry queries. No class — flat utility collection.

## FILE CATALOG

| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `opencode-codex-orch` in `.opencode/opencode.json` plugin array |
...
| `generate-oco-config.ts` | Generate `opencode-codex-orch.json` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `opencode-codex-orch@X.Y.Z` for installation |
| `write-oco-config.ts` | Write generated config to `.opencode/opencode-codex-orch.json` |
...
5. generate-oco-config (build config from user selections)
6. write-oco-config
