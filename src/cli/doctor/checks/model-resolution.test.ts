...
describe("model-resolution check", () => {
  describe("getModelResolutionInfo", () => {
    ...
    it("returns agent requirements with provider chains", async () => {
      ...
      const orchestrator = info.agents.find((a) => a.name === "orchestrator")
      expect(orchestrator).toBeDefined()
      expect(orchestrator!.requirement.fallbackChain[0]?.model).toBe("claude-opus-4-6")
  ...
  describe("getModelResolutionInfoWithOverrides", () => {
    // given: User has overrides in opencode-codex-orch.json
