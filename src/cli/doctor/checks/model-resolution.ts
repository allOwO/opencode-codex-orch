...
import { loadOcoConfig } from "./model-resolution-config"
...
import type { AgentResolutionInfo, CategoryResolutionInfo, ModelResolutionInfo, OcoConfig } from "./model-resolution-types"
...
export function getModelResolutionInfoWithOverrides(config: OcoConfig): ModelResolutionInfo {
  const agents: AgentResolutionInfo[] = Object.entries(AGENT_MODEL_REQUIREMENTS).map(([name, requirement]) => {
