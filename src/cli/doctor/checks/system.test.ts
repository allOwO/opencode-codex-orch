...
const mockGetLoadedPluginVersion = mock(() => ({
  ...
  installedPackagePath: "/tmp/node_modules/opencode-codex-orch/package.json",
...
mock.module("./system-loaded-version", () => ({
  ...
  getSuggestedInstallTag: mock((version: string | null) => (version?.includes("canary") ? "canary" : "latest")),
}))
...
describe("system check", () => {
  ...
  describe("#given cache directory contains spaces", () => {
    ...
    it("uses the loaded version channel for update fix command", async () => {
      ...
      mockGetPluginInfo.mockReturnValue({
        registered: true,
        entry: "opencode-codex-orch@3.0.0-canary.1",
        isPinned: false,
        pinnedVersion: null,
        configPath: null,
        isLocalDev: false,
      ...
      expect(outdatedIssue?.fix).toBe(
        'Update: cd "/Users/test/Library/Caches/opencode with spaces" && bun add opencode-codex-orch@canary'
      )
    })
  })
})
