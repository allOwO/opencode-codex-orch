...
export function getLoadedPluginVersion(): LoadedVersionInfo {
  ...
  const installedPackagePath = join(cacheDir, "node_modules", PLUGIN_PACKAGE_NAME, "package.json")

  const cachePackage = readPackageJson(cachePackagePath)
  const installedPackage = readPackageJson(installedPackagePath)

  const expectedVersion = normalizeVersion(cachePackage?.dependencies?.[PLUGIN_PACKAGE_NAME])
