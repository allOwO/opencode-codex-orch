...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
  lines.push(color.bold(color.white("opencode-codex-orch Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && bun update opencode-codex-orch")}`)
      break
    case "local-dev":
