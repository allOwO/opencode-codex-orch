...
function createTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "oco-run-continuation-"))
  testDirs.push(dir)
  return dir
}
...
describe("checkCompletionConditions continuation coverage", () => {
  it("returns false when active boulder continuation exists for this session", async () => {
    ...
    const planPath = join(directory, "docs", "superpowers", "plans", "active-plan.md")
    mkdirSync(join(directory, "docs", "superpowers", "plans"), { recursive: true })
  ...
  it("returns true when boulder exists but is complete", async () => {
    ...
    spyOn(console, "log").mockImplementation(() => {})
    const directory = createTempDir()
    const planPath = join(directory, "docs", "superpowers", "plans", "done-plan.md")
    mkdirSync(join(directory, "docs", "superpowers", "plans"), { recursive: true })
