...
export async function runCliInstaller(args: InstallArgs, version: string): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo(
      "Usage: bunx opencode-codex-orch install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    )
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Adding opencode-codex-orch plugin...")
  ...
  printStep(step++, totalSteps, "Writing opencode-codex-orch configuration...")
  const omoResult = writeOcoConfig(config)
  ...
  if (!config.hasClaude) {
    ...
    console.log()
    console.log(color.red(color.bold("  The orchestrator is STRONGLY optimized for Claude Opus 4.5.")))
  ...
  console.log(
    `  ${color.dim("gh api --silent --method PUT /user/starred/allOwO/opencode-codex-orch >/dev/null 2>&1 || true")}`,
  )
  console.log()
  console.log(color.dim("oco... Enjoy!"))
  console.log()
