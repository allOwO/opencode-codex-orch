...
Fire ALL of these simultaneously using \`call_oco_agent\`:
...
// Get current diagnostics (directories need extension, e.g. extension=".ts")
...
\`\`\`typescript
// 1. Check diagnostics (directories need extension, e.g. extension=".ts")
lsp_diagnostics(filePath)
