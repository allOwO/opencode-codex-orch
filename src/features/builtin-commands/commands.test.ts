...
describe("loadBuiltinCommands", () => {
  ...
  test("should include skill-creator command in loaded commands", () => {
	//#given
	const disabledCommands: BuiltinCommandName[] = []
	...
	//#then
	expect(commands["skill-creator"]).toBeDefined()
	expect(commands["skill-creator"].name).toBe("skill-creator")
  })

  test("should exclude skill-creator when disabled", () => {
	//#given
	const disabledCommands: BuiltinCommandName[] = ["skill-creator"]

	//#when
	const commands = loadBuiltinCommands(disabledCommands)

	//#then
	expect(commands["skill-creator"]).toBeUndefined()
  })

  test("should include skill-creator template content in command template", () => {
	//#given - no disabled commands
	...
	//#then
	expect(commands["skill-creator"].template).toContain(SKILL_CREATOR_TEMPLATE)
