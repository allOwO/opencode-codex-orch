...
export async function loadAllPluginComponents(options?: PluginLoaderOptions): Promise<PluginComponentsResult> {
  ...
  const [commands, agents, mcpServers, hooksConfigs] = await Promise.all([
  ...
  log(`Loaded ${plugins.length} plugins with ${Object.keys(commands).length} commands, ${Object.keys(agents).length} agents, ${Object.keys(mcpServers).length} MCP servers`)
