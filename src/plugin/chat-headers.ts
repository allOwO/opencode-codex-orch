import { OCO_INTERNAL_INITIATOR_MARKER } from "../shared"
...
async function hasInternalMarker(
  ...
  try {
    ...
    const hasMarker = data.parts.some((part) => {
      ...
      return part.text.includes(OCO_INTERNAL_INITIATOR_MARKER)
    })
...
async function isOcoInternalMessage(input: ChatHeadersInput, client: PluginContext["client"]): Promise<boolean> {
...
export function createChatHeadersHandler(args: { ctx: PluginContext }): (input: unknown, output: unknown) => Promise<void> {
  ...
  return async (input, output): Promise<void> => {
    ...
    if (!(await isOcoInternalMessage(normalizedInput, ctx.client))) return
