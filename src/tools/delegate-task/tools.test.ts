...
function createTestAvailableModels(): Set<string> {
  ...
  describe("category delegation config validation", () => {
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       const mockManager = { launch: async () => ({ id: "task-123", status: "pending", description: "Test task", agent: "executor", sessionID: "test-session" }) }
       const mockClient = {
         app: { agents: async () => ({ data: [] }) },
       ...
       expect(result).not.toContain("opencode-codex-orch requires a default model")
    }, { timeout: 10000 })
  ...
  describe("category variant", () => {
    ...
    test("DEFAULT_CATEGORIES deep model passes to background WITHOUT userCategories", async () => {
      ...
      // when - deep uses the built-in codex model in DEFAULT_CATEGORIES
      await tool.execute(
        {
          description: "Test deep default variant",
          prompt: "Do something",
          category: "deep",
          run_in_background: true,
          load_skills: ["git-commit"],
      ...
      // then - the built-in deep model should be passed with its variant
      expect(launchInput.model).toEqual({
        providerID: "openai",
     ...
     test("DEFAULT_CATEGORIES deep model passes to sync session.prompt WITHOUT userCategories", async () => {
      ...
      await tool.execute(
        {
          description: "Test deep sync variant",
          prompt: "Do something",
          category: "deep",
          run_in_background: false,
          load_skills: ["git-commit"],
      ...
      expect(promptBody.variant).toBe("medium")
    }, { timeout: 20000 })
  })
  ...
  test("sync continuation preserves variant from previous session message", async () => {
    ...
    const tool = createDelegateTask({
      manager: { resume: async () => ({ id: "task-var", sessionID: "ses_var_test", description: "Variant test", agent: "executor", status: "running" }) },
      client: mockClient,
    })
    ...
    expect(callArgs.body.agent).toBe("executor")
    ...
    test("sync mode prompt error returns error message immediately", async () => {
      ...
      const mockManager = {
       ...
       const mockClient = {
         ...
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         app: {
            agents: async () => ({ data: [{ name: "deep", mode: "subagent" }] }),
         },
       }
  ...
  describe("unstable agent forced background mode", () => {
    ...
    test("non-gemini model with run_in_background=false should run sync (not forced to background)", async () => {
      ...
      const mockManager = {
       ...
       const mockClient = {
        ...
        // Use deep which resolves to a non-gemini model
       const tool = createDelegateTask({
         manager: mockManager,
         client: mockClient,
       })
      ...
      // when - using deep (non-gemini model) with run_in_background=false
      const result = await tool.execute(
        {
    ...
    test("visual-engineering category (gemini) with run_in_background=false should force background but wait for result", async () => {
      // given - visual-engineering also uses gemini model
      const { createDelegateTask } = require("./tools")
      let launchCalled = false
      
      const launchedTask = {
        id: "task-visual-engineering",
        sessionID: "ses_visual_engineering_gemini",
        description: "Visual gemini task",
        agent: "executor",
        status: "running",
      }
      const mockManager = {
       ...
       const mockClient = {
         ...
         session: {
           get: async () => ({ data: { directory: "/project" } }),
            create: async () => ({ data: { id: "ses_visual_engineering_gemini" } }),
           prompt: async () => ({ data: {} }),
           promptAsync: async () => ({ data: {} }),
           messages: async () => ({
             data: [
                { info: { role: "assistant", time: { created: Date.now() } }, parts: [{ type: "text", text: "Visual result here" }] }
             ]
           }),
            status: async () => ({ data: { "ses_visual_engineering_gemini": { type: "idle" } } }),
         },
       }
      ...
      // when - visual-engineering category (gemini-3.1-pro with high variant)
      const result = await tool.execute(
        {
          description: "Test visual forced background",
          prompt: "Do something visual",
          category: "visual-engineering",
          run_in_background: false,
          load_skills: ["git-commit"],
      ...
      expect(launchCalled).toBe(true)
      expect(result).toContain("SUPERVISED TASK COMPLETED")
      expect(result).toContain("Visual result here")
    }, { timeout: 20000 })
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO orchestratorJuniorModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        // userCategories: undefined - use DEFAULT_CATEGORIES only
        // orchestratorJuniorModel: undefined
    ...
    test("executor model override takes precedence over category model", async () => {
      // given - executor override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // when - using deep category (default model is openai/gpt-5.3-codex)
      await tool.execute(
        {
          description: "Override precedence test",
          prompt: "Do something",
    ...
    test("explicit category model takes precedence over executor model", async () => {
      // given - explicit category model differs from executor override
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // when - using deep category with explicit model override
      await tool.execute(
        {
          description: "Category precedence test",
          prompt: "Do something",
    ...
    test("executor model override works with quick category (#1295)", async () => {
      // given - user configures agents.executor.model but uses quick category
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // then - executor override model should be used, not category default
      expect(launchInput.model.providerID).toBe("anthropic")
      expect(launchInput.model.modelID).toBe("claude-sonnet-4-6")
    })

    test("executor model override works with user-defined category (#1295)", async () => {
      ...
      // then - executor override model should be used as fallback
      expect(launchInput.model.providerID).toBe("openai")
  ...
  describe("modelInfo detection via resolveCategoryConfig", () => {
    ...
    test("default model is used for category with default entry", () => {
      // given - quick has default model
      const categoryName = "quick"
      ...
      expect(resolved).not.toBeNull()
      expect(resolved!.config.model).toBe("anthropic/claude-haiku-4-5")
    })

    test("category built-in model takes precedence over inheritedModel for builtin category", () => {
      // given - builtin deep category with its own model, inherited model also provided
      const categoryName = "deep"
      const inheritedModel = "cliproxy/claude-opus-4-6"
      ...
      // then - category's built-in model wins (deep uses gpt-5.3-codex)
      expect(resolved).not.toBeNull()
    ...
    test("when user defines model - modelInfo should report user-defined regardless of inheritedModel", () => {
      // given
      const categoryName = "deep"
      const userCategories = { deep: { model: "my-provider/custom-model" } }
      const inheritedModel = "cliproxy/claude-opus-4-6"
    ...
    test("detection logic: actualModel comparison correctly identifies source", () => {
      ...
      const categoryName = "deep"
      const inheritedModel = "cliproxy/claude-opus-4-6"
      const userCategories = { deep: { model: "user/model" } }
    ...
    test("FIXED: category built-in model takes precedence over inheritedModel", () => {
      ...
      // when category has a built-in model (gpt-5.3-codex for deep)
      const resolved = resolveCategoryConfig(categoryName, { inheritedModel, systemDefaultModel: SYSTEM_DEFAULT_MODEL })
    ...
    test("FIXED: userConfig.model always takes priority over everything", () => {
      ...
      const categoryName = "deep"
      const userCategories = { deep: { model: "custom/user-model" } }
      const inheritedModel = "anthropic/claude-opus-4-6"
      const systemDefaultModel = "anthropic/claude-sonnet-4-6"
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured oracle to use a specific model in opencode-codex-orch.json
      const { createDelegateTask } = require("./tools")
      let promptBody: any
  ...
  describe("subagent task permission", () => {
    ...
    test("non-plan subagent should NOT have task permission", async () => {
      //#given - orchestrator delegates to oracle (non-plan)
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      // when - orchestrator delegates to oracle
      await tool.execute(
        {
