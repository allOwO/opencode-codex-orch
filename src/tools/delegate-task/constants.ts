...
You are working on DEEP IMPLEMENTATION / HARD LOGIC tasks.
...
- Fix hairy problems that require deep understanding or architecture judgment
...
1. Launch background agents to gather context:
   - call_oco_agent(description="Explore codebase patterns", subagent_type="explore", run_in_background=true, prompt="<search for relevant patterns, files, and implementations in the codebase related to user's request>")
   - call_oco_agent(description="Research documentation", subagent_type="librarian", run_in_background=true, prompt="<search for external documentation, examples, and best practices related to user's request>")
...
/**
 * Plan family: active `plan` plus retired `prometheus` kept for legacy compatibility.
 * Shares mutual delegation blocking and task tool permission.
