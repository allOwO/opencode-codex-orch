...
const TEST_DIR = join(tmpdir(), `oco-test-session-manager-${randomUUID()}`);
...
describe("session-manager storage - SDK path (beta mode)", () => {
	const mockClient = {
		session: {
			list: mock(() => Promise.resolve({ data: [] as unknown[] })),
			messages: mock(() => Promise.resolve({ data: [] as unknown[] })),
			todo: mock(() => Promise.resolve({ data: [] as unknown[] })),
		},
	};
	...
	test("readSessionMessages supports top-level SDK metadata", async () => {
		// given
		const mockMessages = [
			{
				id: "msg_1",
