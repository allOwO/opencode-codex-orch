...
describe("syncTaskTodoUpdate", () => {
  ...
  it("serializes concurrent updates for the same session", async () => {
    // given
    ...
    let writerCount = 0;
    mockCtx.client.session.todo.mockImplementation(async () => ({ data: currentTodos }));
    ...
    // when
    const firstUpdate = syncTaskTodoUpdate(mockCtx, firstTask, "session-1", writer);
    const secondUpdate = syncTaskTodoUpdate(mockCtx, secondTask, "session-1", writer);
    await writerStarted;

    // then - second update must wait until the first fetch/write cycle finishes
    ...
    expect(currentTodos.find((todo) => todo.id === "T-1")?.status).toBe("completed");
    expect(currentTodos.find((todo) => todo.id === "T-2")?.status).toBe("completed");
  });
});
