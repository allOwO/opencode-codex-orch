...
import { getSDKMessageMetadata } from "../../shared";
import type {
...
export async function formatFullSession(
	...
	const sortedMessages = [...rawMessages].sort((a, b) => {
		const timeA = String(getSDKMessageMetadata(a).time ?? "");
		const timeB = String(getSDKMessageMetadata(b).time ?? "");
	...
	if (options.sinceMessageId) {
		const index = filteredMessages.findIndex(
			(message) => getSDKMessageMetadata(message).id === options.sinceMessageId,
		);
		if (index === -1) {
	...
	for (const message of visibleMessages) {
		...
		const agent = metadata.agent ? ` (${metadata.agent})` : "";
		const time = formatMessageTime(metadata.time);
		const idLabel = metadata.id ? ` id=${metadata.id}` : "";
		lines.push("");
