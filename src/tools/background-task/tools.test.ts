...
const projectDir = "/Users/yeongyu/local-workspaces/opencode-codex-orch";
...
describe("background_output full_session", () => {
	...
	test("shows category instead of agent for executor", async () => {
		// #given
		clearPendingStore();
	...
	test("supports nested info metadata for ids and timestamps", async () => {
		// #given
		const task = createTask();
		const manager = createMockManager(task);
		const client = createMockClient({
			"ses-1": [
				{
					info: { id: "m1", role: "assistant", time: "2026-01-01T00:00:00Z" },
					parts: [{ type: "text", text: "hello" }],
				},
				{
					info: { id: "m2", role: "assistant", time: "2026-01-01T00:00:01Z" },
					parts: [{ type: "text", text: "after" }],
				},
			],
		});
