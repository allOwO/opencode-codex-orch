...
export function loadJsonFile<T>(path: string): T | null {
	...
	try {
		return parseOcoConfigJson(readFileSync(path, "utf-8")) as T;
	} catch {
		return null;
	}
}

function loadOpenCodeConfigFile<T>(path: string): T | null {
	if (!existsSync(path)) return null;
	try {
...
export function getConfigPaths(): {
	...
	return {
		project: resolveExistingProjectOcoConfigFilePath(cwd),
		user: resolveExistingOcoConfigFilePath(configDir),
...
export function loadAllConfigs(): Map<ConfigSource, ConfigJson> {
	...
	const opencode = loadOpenCodeConfigFile<ConfigJson>(paths.opencode);
