...
describe("buildNonClaudePlannerSection", () => {
	const planAgents = [createAgent("plan")];

	it("#given non-Claude model with plan agent #when building #then returns plan agent section", () => {
		//#given
		const model = "google/gemini-3-pro";

		//#when
		const result = buildNonClaudePlannerSection(model, planAgents);
	...
	it("#given GPT model without plan agent #when building #then returns empty", () => {
		//#given
		const model = "openai/gpt-5.4";
...
describe("availability-aware prompt helpers", () => {
	it("falls back to repo-native routing when research agents are unavailable", () => {
		expect(buildResearchAnswerRouting([])).toBe(
			"repo-native tools → synthesize → answer",
		);
		expect(buildInvestigationRouting([])).toBe(
			"repo-native tools → report findings",
		);
		expect(buildResearchToolUsageRules([])).toContain(
			"No research agents available",
		);
		expect(buildBackgroundResearchExamples([])).toContain(
			"No research agents are available",
		);
	});

	it("falls back to repo-native search flow when explore and librarian are unavailable", () => {
		const result = buildDefaultResearchFlow([]);

		expect(result).toContain("repo-native tools");
		expect(result).not.toContain("explore/librarian");
	});

	it("keeps strong explore and librarian guidance when both are available", () => {
		const agents = [createAgent("explore"), createAgent("librarian")];

		expect(buildResearchAnswerRouting(agents)).toBe(
			"explore/librarian → synthesize → answer",
		);
		expect(buildInvestigationRouting(agents)).toBe("explore → report findings");
		expect(buildDefaultResearchFlow(agents)).toContain("explore/librarian");
		expect(buildExplorationParallelGuidance(agents)).toContain(
			"Fire 2-5 explore/librarian agents",
		);
		expect(buildResearchDelegationThresholds(agents)).toContain("`explore`");
		expect(buildResearchDelegationThresholds(agents)).toContain("`librarian`");
		expect(buildResearchToolUsageRules(agents)).toContain(
			"Explore/Librarian = background research",
		);
		expect(buildBackgroundResearchExamples(agents)).toContain(
	...
	it("uses available runtime tool names in search guidance", () => {
		...
		const result = buildSearchGuidance(tools);

		expect(result).toContain("`glob`");
		expect(result).toContain("`grep`");
		expect(result).toContain("`lsp_*`");
