---
description: Publish opencode-codex-orch to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for opencode-codex-orch. Execute the FULL publish workflow from start to finish.
...
[
  ...
  { "id": "verify-platform-binaries", "content": "Verify all 9 platform binary packages published", "status": "pending", "priority": "high" },
...
```bash
npm view opencode-codex-orch version
```
...
After publish-platform workflow completes, verify all 9 platform packages are published:

```bash
PLATFORMS="darwin-arm64 darwin-x64 darwin-x64-baseline linux-x64 linux-x64-baseline linux-arm64 linux-x64-musl linux-x64-musl-baseline linux-arm64-musl"
for PLATFORM in $PLATFORMS; do
  npm view "opencode-codex-orch-${PLATFORM}" version
done
```

All 9 packages should show the same version as the main package (`${NEW_VERSION}`).

**Expected packages:**
| Package | Description |
|---------|-------------|
| `opencode-codex-orch-darwin-arm64` | macOS Apple Silicon |
| `opencode-codex-orch-darwin-x64` | macOS Intel |
| `opencode-codex-orch-darwin-x64-baseline` | macOS Intel baseline |
| `opencode-codex-orch-linux-x64` | Linux x64 (glibc) |
| `opencode-codex-orch-linux-x64-baseline` | Linux x64 baseline (glibc) |
| `opencode-codex-orch-linux-arm64` | Linux ARM64 (glibc) |
| `opencode-codex-orch-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `opencode-codex-orch-linux-x64-musl-baseline` | Linux x64 baseline (musl/Alpine) |
| `opencode-codex-orch-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
...
- New version number
- GitHub release URL: https://github.com/allOwO/opencode-codex-orch/releases/tag/v{version}
- npm package URL: https://www.npmjs.com/package/opencode-codex-orch
- Platform packages status: List all 9 platform packages with their versions
...
<current-context>
<published-version>
!`npm view opencode-codex-orch version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view opencode-codex-orch version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
