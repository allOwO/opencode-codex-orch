# Contributing to opencode-codex-orch

First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to opencode-codex-orch.
...
git clone https://github.com/allOwO/opencode-codex-orch.git
cd opencode-codex-orch
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   For example, if your project is at `/Users/yourname/projects/opencode-codex-orch`:

   ```json
   {
     "plugin": ["file:///Users/yourname/projects/opencode-codex-orch/dist/index.js"]
   }
   ```

   > **Note**: Remove `"opencode-codex-orch"` from the plugin array if it exists, to avoid conflicts with the npm version.
...
opencode-codex-orch/
├── src/
│   ├── index.ts         # Plugin entry (OpenCodeCodexOrchPlugin)
│   ├── plugin-config.ts # JSON multi-level config (Zod v4)
│   ├── agents/          # 10 agents (Sisyphus, Oracle, Librarian, Explore, Atlas, Prometheus, Metis, Momus, Multimodal-Looker, Sisyphus-Junior)
...
Thank you for contributing to opencode-codex-orch! Your efforts help make AI-assisted coding better for everyone.
