...
const PACKAGE_NAME = "opencode-codex-orch"
...
console.log("=== Publishing opencode-codex-orch (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `opencode-codex-orch-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/allOwO/opencode-codex-orch/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/opencode-codex-orch.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
