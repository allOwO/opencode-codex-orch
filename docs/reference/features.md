# opencode-codex-orch Features Reference
...
| **DeepSearch**   | `gpt-5.3-codex`    | Deep research orchestrator. Plans non-overlapping research tracks across external sources, local docs, and local code, dispatches librarian/explore work, synthesizes results in the parent process, and runs a final reviewer quality check before presenting the report. Fallback: `k2p5` → `gemini-3-flash` → `glm-4.6v` → `gpt-5-nano`. |

### Supporting Agents

| Agent         | Model                  | Purpose                                                                                                                                                                                     |
| ------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Reviewer**  | `gpt-5.4`              | Plan and report reviewer — validates clarity, verifiability, evidence coverage, and completeness before or after execution. Fallback: `claude-opus-4-6` → `gemini-3.1-pro`.               |
...
Customize agent models, prompts, and permissions in `opencode-codex-orch.json`.
...
You can define custom categories in `opencode-codex-orch.json`.
...
opencode-codex-orch provides two browser automation providers, configurable via `browser_automation_engine.provider`.
...
Requires the `sg` CLI from `@ast-grep/cli` or a system install such as `brew install ast-grep`.
...
**Note on Claude Code Alignment**: This implementation follows Claude Code's internal Task tool signatures (`TaskCreate`, `TaskUpdate`, `TaskList`, `TaskGet`) and field naming conventions (`subject`, `blockedBy`, `blocks`, etc.). However, Anthropic has not published official documentation for these tools. This is opencode-codex-orch's own implementation based on observed Claude Code behavior and internal specifications.
