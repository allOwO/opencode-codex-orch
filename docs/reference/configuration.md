...
Complete reference for `opencode-codex-orch.json` configuration. This document covers every available option with examples.
...
1. `.opencode/opencode-codex-orch.json`
2. User config:

| Platform    | Path                                      |
| ----------- | ----------------------------------------- |
| macOS/Linux | `~/.config/opencode/opencode-codex-orch.json` |
| Windows     | `%APPDATA%\opencode\opencode-codex-orch.json` |

Plugin config uses strict JSON. Comments and trailing commas are rejected. OpenCode core config still supports `opencode.jsonc`.
...
Run `bunx opencode-codex-orch install` for guided setup. Run `opencode models` to list available models.
